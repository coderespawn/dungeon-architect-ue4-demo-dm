...
AActor* AShooterGameMode::ChoosePlayerStart_Implementation(AController* Player)
{
	...
	AShooterPlayerStart* FoundPlayerStart = NULL;
	UClass* PawnClass = GetDefaultPawnClassForController(Player);
	APawn* PawnToFit = PawnClass ? PawnClass->GetDefaultObject<APawn>() : nullptr;
	TArray<AShooterPlayerStart*> UnOccupiedStartPoints;
	TArray<AShooterPlayerStart*> OccupiedStartPoints;
	for (TActorIterator<AShooterPlayerStart> It(GetWorld()); It; ++It)
	{
		AShooterPlayerStart* PlayerStart = *It;
...
void AShooterGameMode::DefaultTimer()
{
	...
	if (GetWorld()->IsPlayInEditor())
	{
		...
		if (GetMatchState() == MatchState::WaitingToStart && MyGameState->bDungeonCreated)
		{
			StartMatch();
		}
		return;
	...
	if (MyGameState && MyGameState->RemainingTime > 0 && !MyGameState->bTimerPaused)
	{
		...
		if (MyGameState->RemainingTime <= 0)
		{
			...
			else if (GetMatchState() == MatchState::WaitingToStart &&  MyGameState->bDungeonCreated)
			{
				StartMatch();
			}
		}
...
void AShooterGameMode::ServerAssignNewDungeonSeed()
{
	if (HasAuthority()) {
		AShooterGameState* const ShooterGameState = Cast<AShooterGameState>(GameState);
		ShooterGameState->DungeonSeed = FMath::Rand();
	}
}
